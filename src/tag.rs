use crate::ep::Ep;
use crate::ffi::*;
use crate::status_ptr_to_result_owned;
...
impl Ep {
    ...
    pub fn tag_send(
        ...
        tag: u64,
        param: &RequestParam,
    ) -> Result<DataResult, ucs_status_t> {
        status_ptr_to_result_owned(unsafe {
            ucp_tag_send_nbx(
                self.handle,
                data.as_ptr() as _,
                data.len(),
...
mod tests {
    ...
    fn tag_send() {
        ...
        let _send_req = comms
            .ep
            .tag_send_from_vec(send_buff.as_slice(), TAG_FULL, &tag_flags)
            .unwrap();
        let recv_req = comms
            .worker
            .tag_recv_from_slice(recv_buff.as_mut_slice(), TAG_FULL, TAG_FULL, &tag_flags)
            .unwrap()
            .unwrap();
