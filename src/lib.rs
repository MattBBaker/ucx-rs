...
impl OwnedRequest {
    // new assumes that the type has already been error checked.
    #[inline]
    pub fn new(request_handle: *mut std::os::raw::c_void, data: Vec<u8>) -> DataResult {
        let request = Request::new(request_handle);
        match request {
            None => DataResult::Data(data),
            Some(x) => DataResult::Request(OwnedRequest { handle: x, data }),
        }
    }
    ...
    #[inline]
    pub fn check_finished(&self) -> Result<bool, ucs_status_t> {
        self.handle.check_finished()
    }

    pub fn data(&self) -> &[u8] { &self.data }
    pub fn take_data(self) -> Vec<u8> {
        assert!(self.handle.check_finished().unwrap());
        self.data
    }
    // This version of take_data does not check if the request has finished. It is up to the programmer
    // to ensure that the data outlives the request.
    pub unsafe fn take_data_unsafe(self) -> Vec<u8> {
        self.data
    }
}
...
#[inline]
pub fn status_ptr_to_result_owned(ptr: ucs_status_ptr_t, data: Vec<u8>) -> Result<DataResult, ucs_status_t> {
    // This is equivlent to the UCS_PTR_IS_ERR() macro.
    if ptr as usize >= ucs_status_t::UCS_ERR_LAST as usize {
        // The transmute() function is how you access C style memory magic. This function will
        // take the intput pointer and then translate it into i8 and then rust will turn the i8
